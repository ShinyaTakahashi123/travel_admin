...
export default async function AccountEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <h1 className="text-lg font-black mb-5">管理者アカウントの編集</h1>

      <div className="max-w-[640px] bg-card border border-border rounded-2xl p-6.5 flex flex-col gap-4.5">
        <div className="flex items-center gap-3.5">
          <div className="w-14 h-14 rounded-full bg-secondary flex items-center justify-center text-secondary-foreground font-black text-xl">
            {admin.name.charAt(0)}
          </div>
          <div>
            <div className="font-black text-[15px]">{admin.name}</div>
        ...
        <div>
          <div className="text-[11px] font-bold text-muted-foreground mb-1.5">メールアドレス</div>
          <div className="border border-input rounded-lg px-3 py-2.5 text-sm bg-muted text-muted-foreground">
            {admin.email}
          </div>
        ...
        <div>
          <div className="text-[11px] font-bold text-muted-foreground mb-1.5">ステータス</div>
