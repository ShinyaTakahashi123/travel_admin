    ...
    <div>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="font-black text-lg">{planner.name}</div>
              <span
      ...
      <div className="flex gap-4 flex-col md:flex-row">
        <div className="bg-card border border-border rounded-2xl p-5 px-5.5 flex-1">
          ...
          <div className="text-[11px] font-bold text-muted-foreground mb-2">プランニングできる都道府県</div>
          <div className="flex gap-1.5 flex-wrap mb-3.5">
            {planner.serviceAreas.length === 0 && <span className="text-xs text-muted-foreground">未設定</span>}
            {planner.serviceAreas.map((s) => (
              <span key={s.areaId} className="bg-muted text-[#475569] text-[11px] font-bold px-2.5 py-0.5 rounded-full">
                {s.area.name}
              </span>
            ))}
          </div>
          <div className="text-[11px] font-bold text-muted-foreground mb-2">得意なジャンル・テーマ</div>
          <div className="flex gap-1.5 flex-wrap mb-3.5">
            {planner.specialtyTags.length === 0 && <span className="text-xs text-muted-foreground">未設定</span>}
          ...
          {planner.profile && (
            <>
              <div className="text-[11px] font-bold text-muted-foreground mb-2">自己紹介</div>
              <div className="text-xs text-[#475569] leading-relaxed">{planner.profile}</div>
            </>
          )}
        </div>

        <div className="bg-card border border-border rounded-2xl p-5 px-5.5 flex-[1.4]">
          <div className="font-black text-sm mb-3.5">投稿したしおり</div>
          <table className="w-full border-collapse">
            <thead>
              <tr>
                {["タイトル", "エリア", "閲覧数", "ステータス"].map((h) => (
                  <th key={h} className="text-left text-[11px] text-muted-foreground font-bold pb-2 pr-3">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {planner.itineraries.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-t border-muted">
                    <td className="text-xs py-2.5 pr-3">
                    ...
                    <td className="text-xs py-2.5 pr-3">{item.viewCount.toLocaleString()}</td>
                    <td className="py-2.5">
                      <span
                        className="text-[11px] font-bold px-2.5 py-0.5 rounded-full"
                        style={{ background: status.bg, color: status.fg }}
